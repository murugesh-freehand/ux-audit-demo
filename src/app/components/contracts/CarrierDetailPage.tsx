...
function OverviewTab({ carrierId }: { carrierId: string }) {
  ...
  if (!active) return (
    <div className="py-12 text-center text-sm text-gray-400">No contracts on file yet.</div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Agreement summary */}
      <div className="bg-white rounded-xl border border-gray-100 px-5 py-4 grid grid-cols-2 sm:grid-cols-4 gap-x-8 gap-y-4">
        {[
      ...
      {active.keyTerms.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100">
            <p className="text-sm font-semibold text-gray-900">Key Terms</p>
            <p className="text-xs text-gray-400 mt-0.5">Contract provisions that affect pricing and audit outcomes</p>
          </div>
                ...
                {auditTerms.map((term) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <ChevronRight
                        size={14}
                        className={cn("text-gray-300 transition-transform shrink-0 ml-2", expandedTerms.has(term.id) && "rotate-90")}
                      />
                    </div>
                    {expandedTerms.has(term.id) && (
                      <p className="text-xs text-gray-500 mt-2 leading-relaxed pr-4">{term.description}</p>
                    )}
                  </button>
                ))}
              </div>
          ...
          {generalTerms.length > 0 && (
            <div>
              <div className="px-5 py-2 bg-gray-50/80 border-b border-gray-100 border-t border-t-gray-100">
                <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wide flex items-center gap-1.5">
      ...
      {active.fakRules.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
            <p className="text-sm font-semibold text-gray-900">FAK Rules</p>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex flex-wrap gap-1">
                        {rule.classes.map((cls) => (
                          <span key={cls} className="inline-flex items-center rounded border border-gray-200 bg-gray-50 px-1.5 py-0.5 text-xs text-gray-600">{cls}</span>
                        ))}
                      </div>
                    </td>
                    <td className="px-5 py-3 font-semibold text-gray-800">{rule.ratedAs}</td>
                    <td className="px-5 py-3 text-gray-600 text-xs">{rule.minWeight.toLocaleString()} – {rule.maxWeight.toLocaleString()} lbs</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-100">
        <span className="font-mono text-sm font-semibold text-gray-900">{charge.code}</span>
        <span className="text-sm text-gray-500">{charge.name}</span>
        <span className="text-xs text-gray-400 flex-1 truncate">{charge.source}</span>
        <Button
          variant="ghost"
          size="sm"
          className="h-7 px-2.5 text-xs text-gray-500 gap-1 shrink-0"
        >
          <Edit2 size={11} /> Edit
        </Button>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-50 bg-gray-50/50">
              <th className="text-left px-5 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Lane / Region</th>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Value</th>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">UOM</th>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Slab</th>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Currency</th>
              <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase tracking-wide">Effective</th>
              <th className="w-16" />
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {charge.laneRates.map((lr) => (
              <tr key={lr.id} className="hover:bg-gray-50/40 transition-colors group">
                {editingRow === lr.id ? (
                  /* Inline edit row */
                  <>
                    <td className="px-5 py-2 text-xs text-gray-700">{lr.laneRegion}</td>
                    <td className="px-4 py-2">
                      <input
                        defaultValue={lr.value ?? ""}
                        className="w-20 rounded border border-orange-300 bg-orange-50 px-2 py-1 text-xs text-gray-900 focus:outline-none focus:ring-1 focus:ring-[#F06B00]"
                        autoFocus
                      />
                    </td>
                    <td className="px-4 py-2 text-xs text-gray-600">{lr.uom}</td>
                    <td className="px-4 py-2 text-xs text-gray-500">{lr.slab}</td>
                    <td className="px-4 py-2 text-xs text-gray-600">{lr.currency}</td>
                    <td className="px-4 py-2 text-xs text-gray-500">{lr.effective ? fmt(lr.effective) : "—"}</td>
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-1">
                        <Button
                          size="sm"
                          className="h-6 px-2 text-xs gap-1 bg-[#F06B00] hover:bg-orange-600 text-white"
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-6 px-1.5 text-xs text-gray-400"
                          onClick={() => setEditingRow(null)}
                        >
                          <X size={12} />
                        </Button>
                  ...
                  /* Read row */
                  <>
                    <td className="px-5 py-2.5 text-xs text-gray-700 font-medium">{lr.laneRegion}</td>
                    <td className="px-4 py-2.5 text-xs font-semibold text-gray-900">
                      {lr.value !== null ? lr.value : <span className="text-gray-400 font-normal">See table</span>}
                    </td>
                    <td className="px-4 py-2.5 text-xs text-gray-600">{lr.uom}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-500">{lr.slab || <span className="text-gray-300">—</span>}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-600">{lr.currency}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-500">{lr.effective ? fmt(lr.effective) : <span className="text-gray-300">—</span>}</td>
                    <td className="px-4 py-2.5">
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-6 px-2 text-xs text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity gap-1"
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            ...
            className="pl-8 text-sm bg-gray-50 border-gray-200 focus:bg-white h-8"
          />
          {search && (
        ...
        <div className="flex items-center bg-gray-100 rounded-lg p-0.5 gap-0.5 shrink-0">
          <button
            ...
            className={cn(
              ...
              view === "lane" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all",
              view === "charge" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"
            )}
          >
      ...
      {view === "lane" && (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
            <p className="text-xs text-gray-400">{filteredLanes.length.toLocaleString()} lane{filteredLanes.length !== 1 ? "s" : ""} {q && `matching "${search}"`}</p>
            <p className="text-xs text-gray-400">Showing sample — full set has {carrierContracts.find((c) => c.status === "ACTIVE")?.laneCount.toLocaleString() ?? "—"} lanes</p>
          </div>
          {filteredLanes.length === 0 ? (
            <div className="py-10 text-center text-sm text-gray-400">No lanes match "{search}"</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">Route</th>
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Mode</th>
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Ref</th>
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Service</th>
                    <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">Validity</th>
                    ...
                    {chargeCodes.map((code) => (
                      <th key={code} className="text-left px-4 py-2.5 text-xs font-medium text-gray-500 uppercase tracking-wide whitespace-nowrap">
                        {code}
                      </th>
                    ))}
                    <th className="w-16" />
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredLanes.map((lane) => (
                    <tr key={lane.id} className="hover:bg-gray-50/40 transition-colors">
                      <td className="px-4 py-3 text-xs text-gray-800 whitespace-nowrap font-medium">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-gray-600">{lane.ref}</td>
                      <td className="px-4 py-3 text-xs text-gray-600">{lane.service}</td>
                      ...
                      {chargeCodes.map((code) => {
                        ...
                        return (
                          <td key={code} className="px-4 py-3 text-xs text-gray-700 whitespace-nowrap">
                            {ch ? (
                              code === "BASE"
                                ? <span className="font-semibold text-gray-900">{lane.discount}%</span>
                                : laneRate?.value !== null && laneRate?.value !== undefined
                                  ? <span>{laneRate.value}</span>
                                  : <span className="text-gray-400 italic">Table</span>
                            ) : (
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      {carrierContracts.length === 0 ? (
        <div className="py-10 text-center text-sm text-gray-400">No contract versions on file.</div>
      ) : (
            ...
            <div key={c.id}>
              ...
              >
                ...
                <div className="flex items-center gap-2 min-w-[120px]">
                  <span className="text-sm font-semibold text-gray-900">v{c.version}</span>
                ...
                <div className="hidden sm:flex items-center gap-6 text-xs text-gray-500 mr-4">
                  <span><span className="font-medium text-gray-800">{c.laneCount.toLocaleString()}</span> lanes</span>
                  <span><span className="font-medium text-gray-800">{c.chargeCount}</span> charges</span>
                  <span>Uploaded {fmt(c.uploadedAt)}</span>
                </div>

                <ChevronRight size={14} className={cn("text-gray-300 shrink-0 transition-transform", expanded === c.id && "rotate-90")} />
              </button>

              {expanded === c.id && (
                <div className="px-5 pb-4 bg-gray-50/40 border-t border-gray-50">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2 pt-3">Source files</p>
...
export default function CarrierDetailPage() {
  ...
  if (!carrier) return <div className="p-8 text-center text-gray-500">Carrier not found.</div>;
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <h1 className="text-gray-900">{carrier.name}</h1>
              <span className="font-mono text-xs text-gray-400 bg-gray-50 border border-gray-200 rounded px-2 py-0.5">{carrier.scac}</span>
